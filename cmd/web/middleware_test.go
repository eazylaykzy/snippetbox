...
func TestSecureHeaders(t *testing.T) {
	...
	// Create a mock HTTP handler with our ping() function handler that we can pass to our secureHeaders
	// middleware, which writes a 200 status code and "OK" response body.
	next := http.HandlerFunc(ping)

	// Pass the mock HTTP handler to our secureHeaders middleware. Because secureHeaders *returns* a http.Handler we
	// can call its ServeHTTP() method, passing in the http.ResponseRecorder and dummy http.Request to execute it.
